...
namespace Iot.Device.Buzzer.Samples
{
    ...
    internal class MelodyPlayer : IDisposable
    {
        ...
        public void Play(NoteElement note, int tempo, int tonesToTranspose = 0)
        {
            Play(new ListMelodyElement { note }, tempo, tonesToTranspose);
        }
        ...
        private void PlayElement(MelodyElement element)
        {
            ...
            else
            {
                ...
                if (noteElement.PauseDurationAfterPlay == -1)
                {
                    Thread.Sleep((int)(durationInMilliseconds * 0.3));
                }
                else if (noteElement.PauseDurationAfterPlay > 0)
                {
                    Thread.Sleep(noteElement.PauseDurationAfterPlay);
                }
            }
        }
